...
typedef q_hfe_open_loop_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * q_hfe_open_loop(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_hfe_open_loop_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_hfe_open_loop -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(q_hfe_open_loop_DWork.HILReadAnalogTimebase_Task,
        q_hfe_open_loop_P.HILReadAnalogTimebase_Clock, 100.0, -1)
        ;
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(q_hfe_open_loop_DWork.HILReadAnalogTimebase_Task)
          ;
